...
//-------------------------------------------------------------------------------------------------------
			...
			vector <MatrixXd> qmatrix;
				vector <vector<int>> history;
					...
					// MatrixXd homogenousV = U.rowwise().homogeneous();
					// VectorXd fa = VectorXd::Zero(vcount);
					// for(i =0; i< vcount; i++)
					// {
					// 	VectorXd pt(4);
					// 	pt << homogenousV(i,0), homogenousV(i,1), homogenousV(i,2), homogenousV(i,3);
					// 	cout << homogenousV(i,0) << " "<< homogenousV(i,1) << " "<<  homogenousV(i,2) << " "<<  homogenousV(i,3) <<endl;
					...
					// Creating fa for each vertex. For each edge we do fa(i) + fa(j)
					MatrixXd homogenousV = U.rowwise().homogeneous();
					MatrixXd fa = MatrixXd::Zero(E.rows(),2);
					for(int i=0; i<E.rows(); i++)
					{
							VectorXd pt(4);
							pt << (homogenousV(E(i,0),0)+homogenousV(E(i,1),0))/2, (homogenousV(E(i,0),1)+homogenousV(E(i,1),1))/2, (homogenousV(E(i,0),2)+homogenousV(E(i,1),2))/2, (homogenousV(E(i,0),3)+homogenousV(E(i,1),3))/2;
					...
					while(zeroflag)
					{
						...
						cout<<"Collapsing Edge: "<<rowi<<" - ("<<E(rowi,0)<<","<<E(rowi,1)<<") from "<<E(rowi,coli)<<" to "<<E(rowi,colj)<<endl;
						...
						for(int iter=0;iter<history.at(E(rowi,coli)).size();iter++)
						{
							...
							//Vertex Update
							U(E(rowi,colj),0) = (U(E(rowi,colj),0)+U(E(rowi,coli),0))/2;
							U(E(rowi,colj),1) = (U(E(rowi,colj),1)+U(E(rowi,coli),1))/2;

							//Update the qmatrix and the fa of destination.
							// VectorXd pt(4);
							int ind = E(rowi,colj);
							// pt << homogenousV(ind,0), homogenousV(ind,1), homogenousV(ind,2), homogenousV(ind,3);
							qmatrix.at(ind)+= qmatrix.at(E(rowi,coli));
							// fa(ind) = pt.transpose() * qmatrix.at(ind) * pt;
							homogenousV = U.rowwise().homogeneous();
							MatrixXd fa = MatrixXd::Zero(E.rows(),2);
							for(int i=0; i<E.rows(); i++)
							{
									VectorXd pt(4);
